...
//
// 求職者へのメール送信
func SendEmailWithJobSeeker(db *database.DB, firebase usecase.Firebase, sendgrid config.Sendgrid, oneSignal config.OneSignal) func(c echo.Context) error {
	return func(c echo.Context) error {
		var (
			param entity.SendEmailWithJobSeekerParam
		)
		...
		jobSeekerIDStr := formParams.Get("job_seeker_id")
		...
		h := di.InitializeEmailWithJobSeekerHandler(firebase, tx, sendgrid, oneSignal)
		p, err := h.SendEmailWithJobSeeker(param)
		if err != nil {
			tx.Rollback()
			renderJSON(c, presenter.NewErrorJSONPresenter(err))
			return err
...
func GetEmailWithJobSeekerListByJobSeekerID(db *database.DB, firebase usecase.Firebase, sendgrid config.Sendgrid, oneSignal config.OneSignal) func(c echo.Context) error {
	return func(c echo.Context) error {
		var (
		...
		h := di.InitializeEmailWithJobSeekerHandler(firebase, db, sendgrid, oneSignal)
		p, err := h.GetEmailWithJobSeekerListByJobSeekerID(uint(jobSeekerIDInt))
		if err != nil {
			renderJSON(c, presenter.NewErrorJSONPresenter(err))
			return err
		}
