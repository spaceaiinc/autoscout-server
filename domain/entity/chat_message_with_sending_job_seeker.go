...
type ChatMessageWithSendingJobSeeker struct {
	...
	UserType           null.Int  `db:"user_type" json:"user_type"`                       // エージェント（0）or 送客求職者（1）
...
func NewChatMessageWithSendingJobSeeker(
	groupID uint,
	userType null.Int,
	message string,
	packageID string,
	...
	sendAt time.Time, // 送客求職者がLINEを送信した時間を正確に取得するため
...
type SendChatMessageWithSendingJobSeekerLineParam struct {
	GroupID uint   `json:"group_id" validate:"required"` // チャットグループID
	Message string `json:"message" validate:"required"`  // メッセージ
	File    string `json:"file"`
}

type SendChatMessageWithSendingJobSeekerLineImageParam struct {
