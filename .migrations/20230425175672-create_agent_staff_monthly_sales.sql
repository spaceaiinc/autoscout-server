-- 担当者個人の月間の売上情報を管理するテーブル
-- +migrate Up
CREATE TABLE IF NOT EXISTS agent_staff_monthly_sales (
  id INT AUTO_INCREMENT NOT NULL UNIQUE,      -- 重複しないID
  staff_management_id INT NOT NULL,           -- 親テーブルのID
  ...
  PRIMARY KEY(id),
  INDEX idx_agent_staff_monthly_sales_staff_management_id (staff_management_id)
);

ALTER TABLE agent_staff_monthly_sales
  ADD CONSTRAINT fk_agent_staff_monthly_sales_staff_management_id
  FOREIGN KEY(staff_management_id)
  REFERENCES agent_staff_sale_managements (id)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

-- +migrate Down
ALTER TABLE agent_staff_monthly_sales DROP FOREIGN KEY fk_agent_staff_monthly_sales_staff_management_id;

DROP TABLE IF EXISTS agent_staff_monthly_sales;